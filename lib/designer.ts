interface Designer {
  name: string;
  engName: string;
  description: string;
  cartridge: {
    title: string;
    category: string;
    description: string;
    color: string;
  };
}

export const designers: Designer[] = [
  {
    name: '임건',
    engName: 'Yim Gun',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Product Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#b1cae0',
    },
  },
  {
    name: '이윤미',
    engName: 'Lee Yunmi',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Product Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#797979',
    },
  },
  {
    name: '이진희',
    engName: 'Lee Jinhee',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Product Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#8abc69',
    },
  },
  {
    name: '김정재',
    engName: 'Kim Jeoungjae',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#ff0000',
    },
  },
  {
    name: '구가영',
    engName: 'Koo Gayeong',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'UX/UI',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#1f468d',
    },
  },
  {
    name: '김혜진',
    engName: 'Kim Hyejin',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'UX/UI',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#e94e1a',
    },
  },
  {
    name: '박주형',
    engName: 'Park Joohyung',
    description: 'johy@dabyeol.com',
    cartridge: {
      title: '디노스',
      category: 'UX/UI',
      description:
        '재택 근무자와 프리랜서처럼 일을 할 때 공간의 제약을 받지 않는 사람이 늘어나고 있다. 그로 인해 디지털 노마드의 수가 증가하였으며 관심을 가지는 사람 또한 늘어났다.\n새로 유입되는 디지털 노마드가 많은 데에 비해 이들을 도와주는 서비스는 파편화되어 있다. 자신의 성향에 맞는 정보를 얻기까지많은 시간을 쏟아야 하는 문제점이 있다. 이들에게 도움을 주기 위한 서비스, ‘디노스’를 제안한다.',
      color: '#bb9977',
    },
  },
  {
    name: '이은아',
    engName: 'Lee Euna',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'UX/UI',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#cad3a2',
    },
  },
  {
    name: '윤솔비',
    engName: 'Yun Solbi',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Service Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#faf8e9',
    },
  },
  {
    name: '김지언',
    engName: 'Kim Jieon',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Motion Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#ff89ac',
    },
  },
  {
    name: '김플잎',
    engName: 'Kim',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Motion Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#1d1d1d',
    },
  },
  {
    name: '양영하',
    engName: 'Yang Youngha',
    description: '인스타/비핸스 작업계정\nE. gildong1234@gmail.com',
    cartridge: {
      title: 'Title',
      category: 'Motion Design',
      description:
        'Lorem ipsum dolor sit amet consectetur. Id aenean scelerisque eros massa risus facilisis mauris tellus. Turpis elementum in sed sed. Justo pharetra viverra mauris neque commodo.',
      color: '#eca4af',
    },
  },
];
